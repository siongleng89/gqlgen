...
import (
	...
	"github.com/siongleng89/gqlgen/plugin/stubgen"
)
...
In `resolver.go`, add `//go:generate go run generate.go`. Now you can run `go generate ./...` instead of `go run github.com/siongleng89/gqlgen generate` to generate the code.
...
- MutateConfig: Allows a plugin to mutate the config before codegen starts. This allows plugins to add
  ...
  [modelgen](https://github.com/siongleng89/gqlgen/tree/master/plugin/modelgen) for an example
- GenerateCode: Allows a plugin to generate a new output file, see
  [stubgen](https://github.com/siongleng89/gqlgen/tree/master/plugin/stubgen) for an example

Take a look at [plugin.go](https://github.com/siongleng89/gqlgen/blob/master/plugin/plugin.go) for the full list of
