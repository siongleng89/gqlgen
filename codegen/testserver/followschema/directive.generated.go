	...
	for _, k := range fieldsInOrder {
		...
		case "inner":
			...
			if data, ok := tmp.(*InnerDirectives); ok {
				...
				err := fmt.Errorf(`unexpected type %T from directive, should be *github.com/siongleng89/gqlgen/codegen/testserver/followschema.InnerDirectives`, tmp)
				return it, graphql.ErrorOnPath(ctx, err)
			}
		case "innerNullable":
			var err error
		...
		case "thirdParty":
			...
			if data, ok := tmp.(*ThirdParty); ok {
				...
				err := fmt.Errorf(`unexpected type %T from directive, should be *github.com/siongleng89/gqlgen/codegen/testserver/followschema.ThirdParty`, tmp)
				return it, graphql.ErrorOnPath(ctx, err)
			}
		}
	}
